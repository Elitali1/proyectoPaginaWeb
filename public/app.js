...
async function cargarPedidos() {
  ...
  pedidosActivos.forEach(pedido => {
    ...
    const detalleProductos = pedido.productos.map(item => {
      ...
      return `${item.cantidad} x ${nombre} - ${masaTexto}${aclaracionTexto}`;
    }).join('<br>');
...
function renderizarListaProductos() {
  ...
  productosDelPedido.forEach((item, index) => {
    ...
    const li = document.createElement('li');
    li.innerHTML = `
      ${item.cantidad} x ${item.nombre} - ${masaTexto}${aclaracionTexto} - $${formatearPrecio(subtotal)}
...
document.getElementById('btn-agregar-producto').addEventListener('click', () => {
  ...
  const tipoMasa = document.getElementById('tipo-masa').value;
